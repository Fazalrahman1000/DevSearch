...
def createSkills(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            return redirect('account')
    context = {'form':form}
    return render(request, 'users/skills-form.html', context)
...
def inbox(request):
    profile = request.user.profile
    requestMessages = profile.messages.all()
    unreadMessages = requestMessages.filter(is_read=False).count()
    context = {'requestMessages':requestMessages, 'unreadMessages':unreadMessages}
